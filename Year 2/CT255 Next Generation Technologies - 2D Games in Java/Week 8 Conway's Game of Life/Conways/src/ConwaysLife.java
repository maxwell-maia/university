...
public class ConwaysLife extends JFrame implements Runnable, MouseListener, MouseMotionListener {
    ...
    private int lastCellChangedX = 0;
    private int lastCellChangedY = 0;
...
// 3: force an application repaint
    ...
    public void mousePressed(MouseEvent e) {
        ...
        else if (!isGameInProgress && ((x>=475 && x<=625)&&(y>=34 && y<=49)))
        {
            //Load button clicked
            System.out.println("Load clicked");

            //Read the gameState from the save_game text file
            String gameStateText = "";
            try {
                BufferedReader reader = new BufferedReader(new FileReader(saveFileDirectory));
                gameStateText = reader.readLine();
                reader.close();
            }
            ...
            //Using the gameStateText which is a String of 1's and 0's
            // set the gameState
            for(int i = 0; i < 40; i++)
            {
                for(int j = 0; j < 40; j++)
                {
                    //A '1' corresponds to the cell being alive
                    if(gameStateText.charAt(i*40 + j) == '1')
                    {
                        gameState[i][j][currVisibleState] = true;
                    }
                    else
        ...
        else if (!isGameInProgress && ((x>=640 && x<=790)&&(y>=34 && y<=49)))
        {
            //Save button clicked
            System.out.println("Save clicked");

            //Build string representing the gameState. String of 1's and 0's
            String gameStateText = "";
            for(int i = 0; i < 40; i++)
            {
                for(int j = 0; j < 40; j++)
                {
                    if(gameState[i][j][currVisibleState])
                    {
                        gameStateText += "1";
                    }
                    else
            ...
            //Save the gameState to the save_game text file
            try {

                BufferedWriter writer = new BufferedWriter(new FileWriter(saveFileDirectory));
                writer.write(gameStateText);
                writer.close();
            }
            catch (IOException ex)
            {
        ...
        else if (!isGameInProgress)
        {
            ...
            //Set the last cell changed variable so that the drag function knows
            lastCellChangedX = x;
            lastCellChangedY = y;
        }
    ...
    public void mouseDragged(MouseEvent e) {
        if(!isGameInProgress)
        {
            ...
            //Get the cell I am currently doing a drag in
            int x = e.getX() / 20;
            int y = e.getY() / 20;


            if(x != lastCellChangedX || y != lastCellChangedY)
            {
                gameState[x][y][currVisibleState] = !gameState[x][y][currVisibleState];
                lastCellChangedX = x;
                lastCellChangedY = y;
...
// redraw all game objects
        ...
        if(!isGameInProgress)
        {
            ...
            //Load
            g.fillRect(475, 34, 150, 15);
            //Save
            g.fillRect(640, 34, 150, 15);

            g.setColor(Color.BLUE);
            g.drawString("Play", 25, 45);
            g.drawString("Randomise", 190, 45);
            g.drawString("Load", 490, 45);
            g.drawString("Save", 655,45);
        }
...
// flip the buffers
    ...
    public static void main(String[] args)
    {
        ...
        saveFileDirectory = workingDirectory + "\\save_game.txt";
